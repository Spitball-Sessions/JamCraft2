def render_all(con, entities, game_map, fov_recompute, root_console, screen_width, screen_height, colors):
    ...
    if fov_recompute:
        for x, y in game_map:
            ...
            if game_map.fov[x,y]:
                if wall:
                    con.draw_char(x, y, None, fg=None, bg=colors.get('l_wall'))
                elif :
                    con.draw_char(x, y, None, fg=None, bg=colors.get("l_ground"))
                ...
                if wall:
                    con.draw_char(x, y, None, fg=None, bg=colors.get('d_wall'))
                else:
                    con.draw_char(x, y, None, fg=None, bg=colors.get('d_ground'))
            else:
                if wall:
                    con.draw_char(x, y, None, fg=None, bg=colors.get('x_wall'))
